...
{
	// 1. accumulate angle changes
	mYaw += yawDelta;
	mPitch += pitchDelta;

	// 2. limit the pitch angle to approximately ± 89 degrees
	float pitchLimit = XM_PIDIV2 - 0.01f; // XM_PIDIV2 = 90 degrees, the margin is slightly smaller to avoid gimbal
	if (mPitch > pitchLimit)
		mPitch = pitchLimit;
	if (mPitch < -pitchLimit)
		mPitch = -pitchLimit;

	// 3. Recalculating the camera orientation as a quaternion from the accumulated angles (excluding roll)
	XMVECTOR q = XMQuaternionRotationRollPitchYaw(mPitch, mYaw, 0.0f);
	XMStoreFloat4(&orientation, q);

	// 4. Calculating a new view vector
	...
	// 5. Calculating the right and upper vectors of the camera
